...
metaDescription: "Generation is 10 to 15 minutes. Brand checks, numbered proofs, versioning and a delivery receipt are the 3 to 6 hours clients actually pay for."
...
If you run a brand studio and you have been charging $400 to $600 for a single AI-assisted hero image, that number is gone. Clients can see the platform pricing. They know what generation costs. They will push back, and they will be right to.
...
I track time against deliverables in my own projects. When I look at any AI-generated image that ships to a client, the generation itself is 10 to 15 minutes of prompt iteration. The rest of the timeline looks like this:
...
- [Audit record](/modules/audit-log-restore) that proves which version was approved and when

That sequence takes 3 to 6 hours per asset when done properly. None of it is generation. All of it is governance.

## The approval trail is the product

A client who uses Midjourney themselves can generate 200 images in an afternoon. What they cannot do, and what almost no one does without a system built for it, is answer these questions six months later:
...
When I build a [brand asset delivery workflow](/services/brand-visuals), I treat the approval trail as a first-class output. The render is ephemeral. The trail is the thing that protects the client and protects the relationship.
...
The fix is not a new tool. It is a naming convention enforced at the point of handoff, a single folder structure that does not bend for exceptions, and a [review gate](/modules/review-state-machine) that requires a version number before the client ever sees the file. I use a simple numbered scheme: `[project-code]-[asset-type]-[round]-[version]`. It takes one minute to set up and it survives a year of revisions without ambiguity.
