---
title: "bs-hub — agency portal with AI-assisted intake and a 12-state delivery funnel"
slug: "bs-hub"
client: "Broomstick Hub"
role: "Solo build — architecture, engineering, AI integration, brand"
period: "Q1 2026 – present"
summary: "Built and shipped a production client/PM portal handling the full client journey — adaptive AI intake, auto-PM routing, scope lock, project workspace with review state machine, audit log, and daily security-watch cron. The same codebase powers the live demo at demo.gravixar.com."
problem: "Broomstick was running its agency on stitched-together tools — a Notion, a shared Drive, email threads, manual spreadsheet handoffs between admin and PM. Inquiries got lost in the seam between 'admin received it' and 'PM owns it'. Scopes drifted between the discovery call and the contract because nobody was holding the canonical version. There was no audit trail when something went sideways, so disputes ended in 'whose memory wins'. They needed one place where the client journey lived end-to-end."
approach: "Built one portal around a state machine. The inquiry funnel is 12 explicit states with hard transitions — no implicit status, no admins typing free-form notes that might or might not mean 'approved'. Every mutating route writes to a compliance-grade audit log with a per-table safe-restore allowlist; the cancellation flow keeps the row, deletes tasks, and issues a credit ledger entry rather than hard-delete. AI is wired in at three points: the intake wizard at /start uses Claude to adaptively pick the next question and generate a brand brief from the client's website, the PM walks into the discovery call with that context already on screen, and a daily security-watch cron sweeps the audit log for anomalies (mass-signup spikes, DELETE bursts, stuck inquiries) and emails admin if anything trips."
outcome: "Live in production. Onboards new clients in under 10 minutes (was 2-3 days of back-and-forth email). The demo subdomain reseeds 5 canonical scenarios weekly so prospects can click around as a real client before signing. 16 modules, all documented in the repo's MODULES.md so the same shape can be forked for the next client."
metrics:
  - { label: "Time to first project activation", value: "~10 min (was 2-3 days)" }
  - { label: "Inquiry funnel states", value: "12, fully audited" }
  - { label: "Modules", value: "16 — full dependency graph in MODULES.md" }
...
stack:
  ...
  - "Vercel BotID"
cover:
  src: "/graphics/bs-hub-cover.png"
  alt: "bs-hub portal — admin inquiries view"
...
The 16 modules — from `identity-auth` at the base to `security-watch` at the tip — are catalogued in [MODULES.md](https://github.com/...) with a dependency graph that says exactly what depends on what. That document is the recipe: when I spin up a portal for the next client, I work down the list and decide module-by-module what to keep, drop, or customize. A pure task-tracker drops twelve modules. An internal HR tool keeps four.
...
[Beeline](#) — different shape, different domain. Coming soon.

A finance module for bs-hub, mirroring the cancellation pattern: separate API + email template + admin UI + audit hooks. Expense tracking, salaried payroll, monthly reports. Last open loop on the bs-hub roadmap.
